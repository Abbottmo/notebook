...
uint32_t htonl(uint32_t hostlong); //本地字节序转网络字节序 32位
uint16_t htons(uint16_t hostshort); //本地字节序转网络字节序 16位
uint32_t ntohl(uint32_t netlong); //网络字节序转本地字节序 32位
...
int main() {
	...
	while (1) {
		...
		if (pid < 0) {
			...
			close(cfd);	//父进程关闭accept
            //SIGCHLD，在一个进程终止或者停止时，将SIGCHLD信号发送给其父进程
			signal(SIGCHLD, wait_child); //回收子进程
		}
	}
    //子进程 处理
	if (pid == 0) {
...
int  FD_ISSET(int fd, fd_set *set); // 判断文件描述符是否在集合中 1在集合中 
void FD_CLR(int fd, fd_set *set);//从监听的文件描述符移除
void FD_SET(int fd, fd_set *set); // 将监听的文件描述符添加到文件集合中
...
int main() {
	...
	FD_ZERO(&allset);
	FD_SET(listenfd, &allset);//将套接字文件描述符添加到监听集合
	...
	while (1) {
		...
		if (FD_ISSET(listenfd, &rset)) {
			...
			FD_SET(connfd, &allset);//将已连接的发送读写文件描述符加入集合
			if (connfd > maxfd) maxfd = connfd; //修改最大文件描述符
			if (i > maxi) maxi = i;
			if (--nread == 0) continue;//只有一个监听，直接continue
		}
		for (i = 0; i <= maxi; ++i) {
			if ((sockfd = client[i]) < 0) continue;
			if (FD_ISSET(sockfd, &rset)) {//当前的读写文件描述符有新的数据
...
struct pollfd {
    ...
    short events;     /* requested events */ //监听事件合集
    short revents;    /* returned events */  //返回事件合集 
};
...
make         生成.o 和可执行文件
sudo make install  将资源拷贝到系统指定目录
特性 ：基于事件的异步通信模型
...
![image-20201030203327203](img\image-20201030203327203.png)
...
GET  请求指定的页面信息
POST 向指定资源提交数据
HEAD 类似get，返回的响应中没有具体的内容，用于获取报头
...
![image-20201102134254630](img\image-20201102134254630.png)
...
int main(int argc, const char* argv[])
{
    ...
    // 创建事件  STDIN_FILENO 代表终端的输入，表示监听终端输入
...
int main(int argc, char **argv)
{
    ...
    //设置监听回调函数 listener_cb ,发生连接事件会调用listener_cb 函数
